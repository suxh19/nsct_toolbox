...
from nsct_python.filters_torch import efilter2, dfilters, modulate2, parafilters, atrousfilters
from nsct_python.utils_torch import extend2, symext, upsample2df

# Import CUDA implementation of zconv2 if available
try:
    from nsct_python.zconv2_cuda import zconv2_cuda as _zconv2_cuda
...
try:
    from nsct_python.atrousc_cuda import atrousc_cuda as _atrousc_cuda
...
def _atrousc_torch(x: torch.Tensor, f: torch.Tensor, mup: torch.Tensor) -> torch.Tensor:
    ...
    if m <= 0 or n <= 0:
        return torch.zeros(max(0, m), max(0, n), dtype=x.dtype, device=x.device)
...
if __name__ == '__main__':
    from nsct_python.filters_torch import dfilters
