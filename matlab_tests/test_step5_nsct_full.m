...
%% 测试数据准备
% 使用一个简单的测试图像
test_image_small = magic(64);

% 使用一个稍大的测试图像
test_image_medium = magic(128);
...
nlevels1 = [2 3 4];
[c1, s1] = nsctdec(test_image_small, nlevels1);
fprintf('  系数单元数: %d\n', length(c1));
...
fprintf('\n测试案例 2: 2 级分解，方向数 [1 2]\n');
nlevels2 = [1 2];
[c2, s2] = nsctdec(test_image_small, nlevels2);
fprintf('  系数单元数: %d\n', length(c2));
...
fprintf('\n测试案例 3: 4 级分解，方向数 [2 3 3 4]\n');
nlevels3 = [2 3 3 4];
[c3, s3] = nsctdec(test_image_medium, nlevels3);
fprintf('  系数单元数: %d\n', length(c3));
...
afilt = '9-7';   % 金字塔滤波器
try
    [c4, s4] = nsctdec(test_image_small, nlevels4, dfilt, afilt);
    [h, g] = atrousfilters(afilt);
    rec4 = nsctrec(c4, s4, h, g);
...
fprintf('\n测试案例 5: 真实/复杂图像\n');
nlevels5 = [2 3 4];
[c5, s5] = nsctdec(test_image_real, nlevels5);
fprintf('  系数单元数: %d\n', length(c5));
...
save('../test_data/step5_nsct_full.mat', ...
    'test_image_small', 'nlevels1', 'c1', 's1', 'rec1', 'error1', ...
    'nlevels2', 'c2', 's2', 'rec2', 'error2', ...
    'test_image_medium', 'nlevels3', 'c3', 's3', 'rec3', 'error3', ...
    'nlevels4', 'dfilt', 'afilt', 'c4', 's4', 'rec4', 'error4', 'has_custom_filters', ...
    'test_image_real', 'nlevels5', 'c5', 's5', 'rec5', 'error5', 'has_real_image', ...
    '-v7.3');  % 使用 v7.3 格式以支持大文件

fprintf('\n测试数据已保存到 test_data/step5_nsct_full.mat\n');
fprintf('\n========================================\n');
fprintf('完整 NSCT 测试摘要:\n');
fprintf('  案例 1 (64x64, [2 3 4]):       误差 = %e\n', error1);
fprintf('  案例 2 (64x64, [1 2]):         误差 = %e\n', error2);
fprintf('  案例 3 (128x128, [2 3 3 4]):   误差 = %e\n', error3);
