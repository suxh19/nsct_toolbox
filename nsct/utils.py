...
def _ensure_batch_image(x: torch.Tensor) -> Tuple[torch.Tensor, bool]:
    """
    Normalize input tensors to shape [N, H, W] while remembering whether a
    batch dimension was introduced.
    """
    if x.dim() == 2:
        return x.unsqueeze(0), True
    if x.dim() == 3:
        return x, False
    raise ValueError("Expected a 2D or 3D tensor representing images")


def _restore_image_batch(x: torch.Tensor, squeezed: bool) -> torch.Tensor:
    """
    Remove the artificial batch dimension when the original input was 2D.
    """
    if squeezed:
        return x.squeeze(0)
    return x
...
def extend2(x, ru, rd, cl, cr, extmod='per'):
    ...
    """
    x_batch, squeezed = _ensure_batch_image(x)

    if extmod == 'per':
        # Periodic extension using circular padding over batch dimension.
        x_4d = x_batch.unsqueeze(1)  # Shape: [N, 1, H, W]
        padded = F.pad(x_4d, (cl, cr, ru, rd), mode='circular').squeeze(1)
        return _restore_image_batch(padded, squeezed)

    if extmod == 'qper_row':
        # Quincunx periodized extension in row.
        rx = x_batch.size(-2)
        left_slice = x_batch[..., -cl:] if cl > 0 else x_batch[..., :0]
        right_slice = x_batch[..., :cr] if cr > 0 else x_batch[..., :0]
        left_ext = torch.roll(left_slice, rx // 2, dims=-2) if cl > 0 else left_slice
        right_ext = torch.roll(right_slice, -rx // 2, dims=-2) if cr > 0 else right_slice
        y = torch.cat([left_ext, x_batch, right_ext], dim=-1)

        y_4d = y.unsqueeze(1)
        padded = F.pad(y_4d, (0, 0, ru, rd), mode='circular').squeeze(1)
        return _restore_image_batch(padded, squeezed)

    if extmod == 'qper_col':
        # Quincunx periodized extension in column.
        cx = x_batch.size(-1)
        top_slice = x_batch[..., -ru:, :] if ru > 0 else x_batch[..., :0, :]
        bottom_slice = x_batch[..., :rd, :] if rd > 0 else x_batch[..., :0, :]
        top_ext = torch.roll(top_slice, cx // 2, dims=-1) if ru > 0 else top_slice
        bottom_ext = torch.roll(bottom_slice, -cx // 2, dims=-1) if rd > 0 else bottom_slice
        y = torch.cat([top_ext, x_batch, bottom_ext], dim=-2)

        y_4d = y.unsqueeze(1)
        padded = F.pad(y_4d, (cl, cr, 0, 0), mode='circular').squeeze(1)
        return _restore_image_batch(padded, squeezed)
...
def symext(x, h, shift, *, index_dtype: Optional[torch.dtype] = None):
    ...
    extended = x_batch.index_select(-1, col_idx)
    ...
    extended = extended.index_select(-2, row_idx)
    extended = extended[..., : m + p - 1, : n + q - 1]
    return _restore_image_batch(extended, squeezed)
