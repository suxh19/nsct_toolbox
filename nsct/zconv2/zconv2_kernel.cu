...
__global__ void zconv2_kernel(
    ...
    const int plane_stride = s_row_len * s_col_len;
    const float* x_plane = x + batch_idx * plane_stride;
    float* y_plane = y + batch_idx * plane_stride;
    ...
    for (int l1 = 0; l1 < f_row_len; ++l1) {
        ...
        for (int l2 = 0; l2 < f_col_len; ++l2) {
            ...
            sum += x_plane[LINPOS(index_x, index_y, s_col_len)] * 
    ...
    y_plane[LINPOS(n1, n2, s_col_len)] = sum;
}
