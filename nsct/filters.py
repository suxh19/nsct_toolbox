...
from nsct.utils import extend2, qupz, modulate2, resampz
...
def efilter2(
    ...
    # Normalise to [N, H, W] for unified processing.
    squeeze_batch = x_float.dim() == 2
    if squeeze_batch:
        x_float = x_float.unsqueeze(0)
    ...
    xext_4d = xext.unsqueeze(1)  # [N, 1, H, W]
    f_rot = torch.flip(f, dims=[0, 1]).unsqueeze(0).unsqueeze(0)

    y = F.conv2d(xext_4d, f_rot).squeeze(1)
